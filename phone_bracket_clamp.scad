...
hbar_len = 4;   // mm:  the length of the stub of 'handlebar'
bracket_th = 1.5;	//mm
bracket_h  = 30;	//mm
clamp_w = 10;	//mm
clamp_th = 10;	//mm
...
difference() {
	...
	// phone bracket bolt hole.
	translate([0.5*strut_dia,0,bracket_h/2]) 
		rotate(90,[0,1,0]) cylinder(r=screw_dia/2+2*tol,h=clamp_th+hbar_len/2*tol,$fs = 1);

	// phone bracket nut hole - we stack 2 nuts to make a complet cut-out.
	translate([0.5*strut_dia,0,bracket_h/2]) 
		rotate(90,[0,1,0]) color("red") nutHole(screw_dia);
	translate([0.5*strut_dia-2,0,bracket_h/2]) 
		rotate(90,[0,1,0]) color("red") nutHole(screw_dia);
